...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  ...
  if(ifs.is_open()){
    ...
    while (std::getline(ifs,line)){
      ...
      courses.push_back(Course{temp[0],temp[1],temp[2]});
    }
  }  
}
...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  if(ofs.is_open()){
    ...
    for(const auto& elem0 : all_courses){
      if(elem0.quarter != "null"){
        ofs << elem0.title << ',' << elem0.number_of_units << ',' << elem0.quarter << '\n';
      }
    }
  }
for (auto it = all_courses.begin(); it != all_courses.end();) {
    if (it->quarter != "null") {
        it = all_courses.erase(it); // erase 返回下一个有效迭代器
    } else {
        ++it;
    }
}
...
void write_courses_not_offered(std::vector<Course>& unlisted_courses) {
  ...
  if(ofs.is_open()){
    ofs << "Title,Number of Units,Quarter" << '\n';
    for(const auto& elem0 : unlisted_courses){
      ofs << elem0.title << ',' << elem0.number_of_units << ',' << elem0.quarter << '\n';
    }
    for (auto it = unlisted_courses.begin(); it != unlisted_courses.end();) {
      it = unlisted_courses.erase(it); // erase 返回下一个有效迭代器
    }
  }
}
